#!/usr/bin/env python3
"""Plain-SGD wall-clock timing baseline for the 3-multivariate_c4 benchmark."""
...
CASE = "3-multivariate_c4"
OUT_JSON = sgd_baseline.timing_json(HERE)
CACHE_DIR = sgd_baseline.cache_dir(HERE)

# These regime keys are also what update_paper_assets.py looks up.
...
def _regimes():
    _, base = _base_config()
    ((width, depth),) = base.architecture_settings
    regimes = []
    for precision, regime_key in _PRECISION_REGIME.items():
        regimes.append(
            (
                regime_key,
                {
